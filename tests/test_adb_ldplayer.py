...
class TestADBLDPlayerIntegration:
    ...
    def test_query_resolution_dumpsys_fallback(self, mock_run):
        ...
        def side_effect(*args, **kwargs):
            call_count[0] += 1
            if call_count[0] == 1:  # wm size call fails
                return MagicMock(returncode=1, stdout="")
            else:  # dumpsys call succeeds
                return MagicMock(
                    returncode=0,
        ...
        assert res == (1080, 1920)
        assert mock_run.call_count == 2  # First wm size, then dumpsys
    ...
    def test_multiple_workers_different_resolutions(self, mock_run):
        ...
        def get_resolution(*args, **kwargs):
            device_id = args[1] + args[2] + args[3] if len(args) > 3 else str(args)
            
            if "emulator-5554" in str(kwargs.get('args', args)):
                return MagicMock(returncode=0, stdout="Physical size: 540x960\n")
            elif "emulator-5555" in str(kwargs.get('args', args)):
